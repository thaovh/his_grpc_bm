...
/**
 * ExpMestOther Entity - Phiếu xuất thuốc khác từ HIS GetView2
 * Table: EXP_EXP_MEST_OTHER (19 chars - OK)
 * Service Prefix: EXP
 */
@Entity('EXP_EXP_MEST_OTHER')
@Index('IDX_EXP_MEST_OTHER_CODE', ['expMestCode'])
@Index('IDX_EXP_MEST_OTHER_DATE', ['createDate'])
@Index('IDX_EXP_MEST_OTHER_WRK_ST', ['workingStateId'])
@Unique('UQ_EXP_MEST_OTHER_HIS_ID', ['hisExpMestId'])
export class ExpMestOther extends BaseEntity {
  // Primary Key từ HIS
  @Column({
    type: 'number',
    precision: 19,
    ...
    nullable: false,
    comment: 'Primary key from HIS system (ID from GetView2)'
  })
  hisExpMestId: number;
  ...
  @Column({
    type: 'varchar2',
    length: 10,
    name: 'TDL_PATIENT_DISTRICT_CODE', // 25 chars - OK
    nullable: true,
    comment: 'Patient district code'
  })
