...
import { ExpMest } from './exp-mest.entity';

/**
 * ExpMestMedicine Entity - Chi tiết đơn xuất thuốc từ HIS
 * Table: EXP_EXP_MEST_MEDICINE (20 chars - OK)
 * Service Prefix: EXP
 */
@Entity('EXP_EXP_MEST_MEDICINE')
@Index('IDX_EXP_MEST_MED_EXP_ID', ['expMestId'])
@Index('IDX_EXP_MEST_MED_MED_ID', ['medicineId'])
@Index('IDX_EXP_MEST_MED_STOCK', ['mediStockId'])
export class ExpMestMedicine extends BaseEntity {
  ...
  // Foreign Key đến ExpMest (HIS ID)
  @Column({
    type: 'number',
    precision: 19,
    scale: 0,
    ...
    nullable: false,
    comment: 'Reference to EXP_EXP_MEST.EXP_MEST_ID (HIS ID)'
  })
  expMestId: number;

  // Foreign Key đến ExpMest (Local UUID) - Optional, for faster joins
  @Column({
    type: 'varchar2',
    length: 36,
    name: 'EXP_MEST_LOCAL_ID', // 18 chars - OK
    nullable: true,
    comment: 'Reference to EXP_EXP_MEST.ID (UUID) for faster local joins'
  })
  ...
  @Column({
    ...
    precision: 19,
    scale: 0,
    name: 'EXP_MEST_ID_BK', // 14 chars - OK
    nullable: true,
    comment: 'Backup export medicine ID'
  })
  expMestIdBk: number | null;
  ...
  // Relationship với ExpMest (Many-to-One)
  // Note: Join qua local UUID (expMestLocalId) nếu có, nếu không thì join qua HIS ID (expMestId)
  @ManyToOne(() => ExpMest, expMest => expMest.medicines, {
